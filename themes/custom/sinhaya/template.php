 ...
 */
function sinhaya_preprocess_node(&$vars) {
  $node = $vars['node'];
  $classes = array();
  $classes[] = 'node';
 ...
 */
function sinhaya_comment_submitted($variables) {
 ...
 */
function sinhaya_theme($existing, $type, $theme, $path) {
  return array(
 ...
 */
function sinhaya_preprocess_comment(&$variables) {
 ...
 */
function sinhaya_dynamic_formatters_style($variables) {
 ...
 */
function sinhaya_preprocess_field(&$variables) {
  ...
  if ($variables['element']['#field_type'] == 'file' && $variables['element']['#formatter'] == 'file_rendered') {
    ...
    drupal_add_js($cycle);
    drupal_add_js(drupal_get_path('theme', 'sinhaya') . '/js/media_slideshow.js');
  }
}